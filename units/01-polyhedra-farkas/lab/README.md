...
1. Projecting a polyhedron gives a polyhedron. Fourier–Motzkin elimination
   computes that projection, and the row count explodes.
...
|---|---|
| Rows as `(a, b)` = *a·x ≤ b*, exact integers | Fourier–Motzkin elimination of one variable |
...
the rows; `itertools.product(pos, neg)` pairs them; `dict.fromkeys` deduplicates
while keeping order.

Split the rows by the sign of `a[k]`. Rows with `a[k] = 0` pass through. Every
row with a positive coefficient pairs with every row with a negative coefficient.
Scale both by positive numbers so the `x_k` terms cancel, and add them.
...
subtract two rows the way Gaussian elimination does? (Inequalities only survive
...
Eliminate *x₀, …, x₍ₙ₋₁₎* in turn. Every remaining row reads *0 ≤ c*. The
...
For every *n*-subset of rows, solve them as equalities with `solve_exact`. Keep
...
pyramid whose apex is tight on four rows but listed once, and agreement with
qhull on random polytopes. Every vertex you return is also checked to have tight
rows of full rank.
...
*Shape: the step-2 fold on augmented rows, then a search.* `next(...)` with a
default of `None` is the functional "first match or nothing".

Every row `eliminate` produces is a nonnegative combination of the original rows.
If you eliminate everything and a row *0 ≤ c* with *c* < 0 is left, the
...
one violated, and that row carries its certificate. This is the proof on slide
...
1. **Blow-up.** Rows after each FM step, next to the number of *true* facets of
   each projection, which qhull computes. The reference run went 26 → 64 → 601 →
   70 996 rows while the true facet count stayed between 29 and 39. Nearly
   everything FM generates is redundant. Keeping only the useful rows is a
...
2. **Vertices.** Your C(*m*, 3) enumeration against qhull as *m* grows. You get
   the same answer, and the reference run was 58× to 5 000× slower. Note that
...
3. **A proof.** One infeasible system, your certificate printed as "multiply
   these rows by these numbers and add", and HiGHS's verdict next to it. HiGHS
